...
import { Viewer, Ion, Camera, Rectangle, BingMapsImageryProvider, BingMapsStyle,WebMapServiceImageryProvider,OpenStreetMapImageryProvider,
  ...
  CallbackProperty, PolylineGraphics, PolylineGeometry, PolygonGeometry, PointGraphics,Cartographic,
  createWorldTerrain, PolygonHierarchy, CzmlDataSource, Math as CesiumMath, HeadingPitchRoll, Transforms } from 'cesium';  
...
//register(proj4);
  ...
  console.log('long: %s, lat: %s',CesiumMath.toDegrees(positionCartographic.longitude), CesiumMath.toDegrees(positionCartographic.latitude));

  viewer.pickTranslucentDepth = true;
      const locationMarker = viewer.entities.add({
      name : 'location',
      position : Cartesian3.fromRadians(positionCartographic.longitude, positionCartographic.latitude, 10),
      point : {
          pixelSize : 5,
          color : Color.RED,
...
export default {
  ...
  mounted() {
    ...
    Camera.DEFAULT_VIEW_RECTANGLE = Rectangle.fromDegrees(126.725305, 37.433627, 127.249917, 37.660199);
...
<style src="@/assets/cesium/Widgets/widgets.css">
</style>
